...
class FigureList {
    ...
    this.text.on(
      'scrollstop', () => {
        ...
        if (exists(this.selected_header)) {
          this.push_hash('#' + this.selected_header
            .attr('id'))
        }
        this.is_skip_auto_figure_next_scroll = false
      })
  ...
  make_toc() {
    ...
    for (let header_dom of $.makeArray(this.text.find(
      ...
      let finish = () => {
        ...
        this.select_onscreen_figlink_and_figure()
        this.push_hash('#' + this.selected_header.attr(
          'id'))
      }
      headerlink.click(
        (e) => {
          e.preventDefault()
          this.scroll_to_href_in_text(header_href,
            finish)
        }
      )
  ...
  make_figlinks() {
    ...
    let n_figlink = 1
    this.figlinks = []
    let figlink_doms = $.makeArray(this.text.find(
      'a[href*="fig"]'))
  ...
  make_reflinks() {
    ...
    let n_reflink = 0
    let reflink_doms = $.makeArray($(this.text_href).find(
      'a[href*="ref"]'))
    for (let reflink_dom of reflink_doms) {
      ...
      if (this.ref_hrefs.indexOf(ref_href) !== -1) {
        ...
        reverse_link.click((e) => {
          e.preventDefault()
          this.is_skip_auto_figure_next_scroll =
            true
          this.scroll_to_href_in_text(
            reflink_href, () => {})
        })
...
class Page {
  ...
  constructor() {
    ...
    this.navbar.click((e) => {
      if ($(e.target).attr('id') !== "toc-trigger") {
        this.shutdown_trigger()
      }
    })
    this.text.click((e) => {
      if ($(e.target).attr('id') !== "toc-trigger") {
    ...
    this.isDrag = 0;
    this.divider1.mousedown((e) => {
      e.preventDefault();
      this.isDrag = 1;
    })
    this.divider2.mousedown((e) => {
      e.preventDefault();
      this.isDrag = 2;
    })
    body.mouseup((e) => {
  ...
  mousemove(e) {
    ...
    if (this.get_n_column() == 2) {
      if (this.isDrag === 2) {
        ...
        console.log(e.clientX, this.split_diff,
          half_window_width)
  ...
  make_resize_fn(img_dom, width) {
    return () => {
      ...
      if (img.hasClass('inline-graphic'))
        return
      if (img_dom.naturalWidth > 0 && img_dom.naturalWidth <
        width)
        img.css('width', '')
      else
        img.css('width', '100%')
  ...
  resize_imgs_in_figlist() {
    ...
    for (let img_dom of $.makeArray($('img'))) {
      ...
      let parent_width = figlist_width - get_spacing_width(
        parent)
      let resize_fn = this.make_resize_fn(img_dom,
        parent_width)
      if (!img_dom.complete) {
        img.load(resize_fn)
      } else {
  ...
  resize_youtube_iframe(iframe) {
    ...
    var width = get_outer_width(parent) - get_spacing_width(
      parent)
  ...
  resize_window() {
    ...
    if (this.get_n_column() == 1) {
      ...
      var figlist_width =
        window_width - body_padding_left -
        body_padding_right - get_outer_width(this.toc) -
    ...
    if (this.get_n_column() == 1) {
      for (let iframe_dom of $.makeArray($(
          '.fig-in-text iframe[src*="youtube.com"]'
        )))
        this.resize_youtube_iframe($(iframe_dom))
    } else {
      for (let iframe_dom of $.makeArray($(
          '.fig-in-figlist iframe[src*="youtube.com"]'
        )))
        this.resize_youtube_iframe($(iframe_dom))
    }
